...
// SectorEconomicoId y EstadoEmpresaId son referencias LÓGICAS a parametro.parametro
// (tipos SECTOR_ECONOMICO y ESTADO_EMPRESA); sin FK (C-6, virtualización).
// Las empresas llegan ya aprobadas desde Ágora: no hay aprobación interna (v4).
type Empresa struct {
...
func GetAllEmpresa(query map[string]string, fields []string, sortby []string,
	...
	qs := orm.NewOrm().QueryTable(new(Empresa)) // sin rel(fk) tras quitar usuario_aprobador (v4)
	var l []Empresa
