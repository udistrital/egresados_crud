...
type Empresa struct {
	...
	SectorEconomico   *SectorEconomico `orm:"column(sector_economico_id);rel(fk);null" json:"sector_economico,omitempty"`
	EstadoEmpresa     *EstadoEmpresa `orm:"column(estado_empresa_id);rel(fk)" json:"estado_empresa"`
...
func GetEmpresaById(id int) (v *Empresa, err error) {
	...
	if err = o.Read(v); err == nil {
		o.LoadRelated(v, "EstadoEmpresa")
		o.LoadRelated(v, "SectorEconomico")
		return v, nil
	}
	return nil, err
}

func GetAllEmpresa() (ml []Empresa, err error) {
	o := orm.NewOrm()
	_, err = o.QueryTable(new(Empresa)).Filter("Activo", true).RelatedSel().All(&ml)
	return
}
